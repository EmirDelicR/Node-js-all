...
class Feed extends Component {
  ...
  finishEditHandler = postData => {
    ...
    if (this.state.editPost) {
      fileConfigSetup.data.oldPath = this.state.editPost.imagePath;
    }

    const configFile = setServerConfigFILE(fileConfigSetup);
    const auth = setAuthorization(this.props.token);
    configFile.headers = auth.headers;

    fetch(process.env.REACT_APP_FILE + "post-image", configFile)
      .then(res => {
        return res.json();
      })
      .then(fileResData => {
        ...
        return fetch(process.env.REACT_APP_GRAPHQL_ROUTE, config).then(res => {
          return res.json();
        });
      })
      .then(resData => {
